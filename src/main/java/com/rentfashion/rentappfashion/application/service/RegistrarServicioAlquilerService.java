...
public class RegistrarServicioAlquilerService  implements RegistrarServicioAlquilerUseCase {
    ...
    public Long registrarServicio(String clienteId, String empleadoId, List<String> refsPrenda, LocalDate fechaAlquiler) {
        if (clienteId == null || clienteId.isBlank()) throw new ValidationException("clienteId es obligatorio");
        if (empleadoId == null || empleadoId.isBlank()) throw new ValidationException("empleadoId es obligatorio");
        if (fechaAlquiler == null) throw new ValidationException("fechaAlquiler es obligatoria");
        if (refsPrenda == null || refsPrenda.isEmpty()) throw new ValidationException("Debe incluir al menos 1 prenda");
        ...
        for (String ref : refsPrenda) {
            if (ref == null || ref.isBlank()) throw new ValidationException("Referencia de prenda inválida");
            Prenda prenda = prendaRepo.findByRef(ref.trim())
                    .orElseThrow(() -> new NotFoundException("Prenda no existe: " + ref));

            if (prenda.getEstado() == Prenda.Estado.EN_LAVANDERIA) {
                throw new BusinessRuleException("La prenda " + ref + " está en lavandería y no puede alquilarse");
            }

            disponibilidadPolicy.asegurarDisponible(ref.trim(), fechaAlquiler);
            prendas.add(prenda);
        }
